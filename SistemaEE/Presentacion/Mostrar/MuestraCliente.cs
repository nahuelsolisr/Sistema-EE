    ...
    public partial class MuestraCliente : MaterialForm
    {
        ...
        private void Seleccionar(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == btn_seleccionar.Index)
            {
                ...
                Clases.Datos.CUITCliente = Convert.ToDecimal(dgvCliente.Rows[e.RowIndex].Cells["Column0"].Value);
                Clases.Datos.NombreCliente = Convert.ToString(dgvCliente.Rows[e.RowIndex].Cells["Column1"].Value);
                Clases.Datos.direccion_cliente = Convert.ToString(dgvCliente.Rows[e.RowIndex].Cells["Column2"].Value);
                Clases.Datos.mail_cliente = Convert.ToString(dgvCliente.Rows[e.RowIndex].Cells["Column3"].Value);
                Clases.Datos.condicionante = Convert.ToString(dgvCliente.Rows[e.RowIndex].Cells["Column4"].Value);
